...
fn main() {
    ...
    let withdraw_amount: u128 = env::read();
    let change_note = read_note();

    assert!(withdraw_amount > 0);

    let owner_modulus = owner_modulus.unwrap();
    assert_eq!(change_note.owner_modulus, owner_modulus);

    let output_leaf = note_leaf(&change_note);

    let expected_total = withdraw_amount.checked_add(change_note.value).unwrap();
    assert_eq!(input_sum, expected_total);

    for nullifier in &nullifiers {
        assert_ne!(*nullifier, change_note.nullifier);
    }
    ...
    verify_owner_signature(owner_modulus, &action_hash, &signature_bytes);

    let journal = WithdrawJournal {
        root,
        nullifiers,
        withdraw_amount,
        output_leaf,
